...
Eluan is a React design system distributed as a pnpm-workspace monorepo. Consumers install `@eluan/core` (UI and marketing components) and `@eluan/tokens` (tokens/CSS). `@eluan/web` is a compatibility facade for Header/HeaderNavigation/Footer, and native-only components live in `@eluan/native`.
