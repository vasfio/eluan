...
Ragnar is a React design system distributed as a pnpm-workspace monorepo. Consumers install `@vasf/ragnar-core` (UI and marketing components) and `@vasf/ragnar-tokens` (tokens/CSS). `@vasf/ragnar-web` is a compatibility facade for Header/HeaderNavigation/Footer, and native-only components live in `@vasf/ragnar-native`.
