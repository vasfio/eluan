...
const AIFilterChip = React.forwardRef<HTMLButtonElement, AIFilterChipProps>(
  (
    ...
    >
      ...
      <span>{children}</span>
      {count !== undefined && (
        <Badge variant="secondary" className="ml-[var(--spacing-xxs)] min-w-[1.25rem] h-[1.25rem] justify-center rounded-full px-[var(--spacing-xxs)] text-[var(--font-size-xs)] leading-none">
          {count}
        </Badge>
      )}
    </Toggle>
...
const AIFilterGroup = React.forwardRef<HTMLDivElement, AIFilterGroupProps>(
  ({ className, label, children, ...props }, ref) => (
    <div
      ...
      {...props}
    >
      <span className="text-[var(--font-size-xs)] text-[var(--container-fg-alt)] font-medium">
        {label}
      </span>
