...
const AIFollowUp = React.forwardRef<HTMLDivElement, AIFollowUpProps>(
  ({ className, variant, heading, children, ...props }, ref) => (
    <div ref={ref} className={cn("w-full", className)} {...props}>
      {heading && (
        <p className="text-[var(--font-size-xs)] text-[var(--container-fg-alt)] uppercase tracking-wide font-medium mb-[var(--spacing-sm)]">
          {heading}
        </p>
      )}
...
const aiFollowUpItemVariants = cva(
  "flex items-center gap-[var(--spacing-xs)] text-[var(--font-size-sm)] text-[var(--container-fg)] transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--interactive-border)] disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
...
>(({ className, variant, hideIcon = false, children, ...props }, ref) => (
  ...
  >
    {!hideIcon && (
      <ArrowRight className="h-[var(--size-xxs)] w-[var(--size-xxs)] shrink-0 text-[var(--container-fg-alt)]" />
    )}
