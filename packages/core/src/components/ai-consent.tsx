...
const aiConsentVariants = cva(
  "border bg-[var(--container-bg)] text-[var(--container-fg)]",
  {
    variants: {
      variant: {
...
const AIConsentItemsList = ({
  ...
  <ul className="list-disc pl-[var(--spacing-md)] text-[var(--font-size-sm)] text-[var(--container-fg-alt)]">
    {items.map((item) => (
      <li key={item}>{item}</li>
    ))}
  </ul>
...
const AIConsent = React.forwardRef<HTMLDivElement, AIConsentProps>(
  (
    ...
    const shieldIcon = (
      <Shield className="h-5 w-5 shrink-0 text-[var(--informative-fg-strong)]" />
    )
        ...
        <Card ref={ref} className={cn("border", className)} elevation="sm" {...props}>
          <CardHeader className="flex-row items-start gap-[var(--spacing-sm)]">
            ...
            <div className="flex flex-col gap-[var(--spacing-xs)]">
              <CardTitle className="text-[var(--font-size-sm)]">{title}</CardTitle>
        ...
        <Dialog open>
          <DialogContent ref={ref} className={className} {...(props as any)}>
            <DialogHeader className="flex-row items-start gap-[var(--spacing-sm)]">
              {shieldIcon}
              <div className="flex flex-col gap-[var(--spacing-xs)]">
                <span className="font-medium text-[var(--font-size-sm)]">
                  {title}
                </span>
                {description && (
                  <p className="text-[var(--font-size-sm)] text-[var(--container-fg-alt)]">
                    {description}
                  </p>
                )}
              </div>
