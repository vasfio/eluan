    ...
    return (
      ...
      >
        {showIcon && (
          <GitBranch
            className={cn(
              "shrink-0 text-[var(--container-fg-alt)]",
        ...
        <span
          className={cn(
            "select-none tabular-nums text-[var(--container-fg)]",
            isCompact
...
const AIBranch = React.forwardRef<HTMLButtonElement, AIBranchProps>(
  ({ className, index, active = false, ...props }, ref) => (
    <button
      ...
      type="button"
      className={cn(
        "inline-flex items-center justify-center rounded-[var(--curves-sm)] px-[var(--spacing-xs)] py-[var(--spacing-xxs)] text-[var(--font-size-xs)] transition-colors",
        active
          ? "bg-[var(--interactive-bg-selected)] text-[var(--interactive-fg-selected)]"
          : "text-[var(--container-fg-alt)] hover:bg-[var(--interactive-bg-hover)] hover:text-[var(--container-fg)]",
        className
      )}
