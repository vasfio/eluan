      ...
      <CardContent className="flex flex-row items-start gap-[var(--spacing-sm)] p-[var(--spacing-sm)]">
        ...
        <div className="shrink-0 mt-[var(--spacing-xxs)]">
          {favicon ? (
            ...
            <Globe className="h-[var(--size-xxs)] w-[var(--size-xxs)] text-[var(--container-fg-alt)]" />
          )}
        </div>

        {/* Content */}
        <div className="flex flex-col flex-1 min-w-0">
          <span className="text-[var(--font-size-sm)] font-medium text-[var(--container-fg)] truncate">
            {title}
          </span>
          {(url || domain) && (
            <span className="text-[var(--font-size-xs)] text-[var(--container-fg-alt)] truncate">
              {domain ?? url}
            </span>
          )}
          {snippet && (
            <p className="mt-[var(--spacing-xxs)] text-[var(--font-size-xs)] text-[var(--container-fg-alt)] line-clamp-2">
              {snippet}
            </p>
          )}
        </div>
...
const AICitationList = React.forwardRef<HTMLDivElement, AICitationListProps>(
  ({ className, heading = "Sources", children, ...props }, ref) => (
    ...
    >
      {heading && (
        <h4 className="text-[var(--font-size-sm)] font-medium text-[var(--container-fg)]">
          {heading}
        </h4>
      )}
      {children}
