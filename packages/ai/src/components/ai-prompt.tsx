...
const aiPromptVariants = cva(
  "relative flex flex-col rounded-[var(--curves-lg)] border border-[var(--container-border-alt)] bg-[var(--container-bg)] text-[color:var(--container-fg)] transition-shadow focus-within:ring-1 focus-within:ring-[var(--interactive-border-alt)]",
  {
    variants: {
      size: {
  ...
  ({ className, onChange, ...props }, ref) => {
    ...
    return (
      <textarea
        ...
        rows={1}
        className={cn(
          "w-full resize-none bg-transparent text-[length:var(--font-size-sm)] placeholder:text-[color:var(--foregrounds-tertiary)] focus:outline-none disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
...
const AIPromptFooter = React.forwardRef<HTMLDivElement, AIPromptFooterProps>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "pt-[var(--spacing-xs)] text-[length:var(--font-size-xs)] text-[color:var(--foregrounds-tertiary)]",
        className
      )}
      {...props}
    />
