...
const aiRegenerateVariants = cva(
  "inline-flex items-center justify-center gap-[var(--spacing-xs)] whitespace-nowrap rounded-[var(--curves-md)] text-[length:var(--font-size-sm)] font-medium transition-all focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--interactive-border)] focus-visible:ring-offset-1 disabled:pointer-events-none disabled:opacity-50 cursor-pointer",
  {
    variants: {
      variant: {
        default:
          "border border-[var(--action-secondary-border)] bg-transparent text-[color:var(--action-secondary-fg)] hover:bg-[var(--action-secondary-bg-hover)] active:bg-[var(--action-secondary-bg-active)]",
        ghost:
          "bg-transparent text-[color:var(--container-fg-alt)] hover:bg-[var(--interactive-bg-hover)] hover:text-[color:var(--container-fg)]",
        primary:
          "bg-[var(--action-primary-bg)] text-[color:var(--action-primary-fg)] hover:bg-[var(--action-primary-bg-hover)] active:bg-[var(--action-primary-bg-active)] shadow-sm",
      },
      size: {
        sm: "h-[var(--size-md)] px-[var(--spacing-sm)] text-[length:var(--font-size-xs)]",
        default: "h-[var(--size-lg)] px-[var(--spacing-md)]",
        lg: "h-[var(--size-xl)] px-[var(--spacing-lg)] text-[length:var(--font-size-base)]",
      },
    },
    defaultVariants: {
      variant: "default",
  ...
  (
    ...
    const mainButton = (
      ...
      >
        ...
        {children ?? "Regenerate"}
        {count && (
          <span className="text-[color:var(--container-fg-alt)] text-[length:var(--font-size-xs)]">
            {count}
          </span>
        )}
      </Button>
