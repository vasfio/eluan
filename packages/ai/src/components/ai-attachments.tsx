...
import { Badge, Button } from "@vasf/ragnar-core"
    ...
    return (
      <span ref={ref as React.Ref<HTMLSpanElement>} {...props}>
        <Badge
          ...
          aria-label={filename}
        >
          <Icon className="h-[var(--size-xxs)] w-[var(--size-xxs)] shrink-0 text-[color:var(--container-fg-alt)]" />
          <span className="truncate">{filename}</span>
          {size && (
            <span className="shrink-0 text-[color:var(--container-fg-alt)]">
              {size}
            </span>
          )}
        </Badge>
...
>(({ className, children, ...props }, ref) => (
  <Button
    ...
    size="sm"
    className={cn(
      "border-dashed text-[color:var(--container-fg-alt)] hover:text-[color:var(--container-fg)]",
      className
    )}
    {...props}
  >
