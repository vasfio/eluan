...
import { View, Text, Pressable, Modal, FlatList, StyleSheet, useColorScheme, } from "react-native";
import { spacing, radii, fontSizes } from "@ragnar/tokens";
export function NativePicker({ options, value, onChange, placeholder = "Select an option", title, disabled = false, style, textStyle, renderTrigger, }) {
    ...
    function PickerModal() {
        ...
        <Pressable style={styles.backdrop} onPress={() => setVisible(false)}>
          <View style={[styles.modalContent, { backgroundColor }]}>
            {title && (<View style={[styles.modalHeader, { borderBottomColor: borderColor }]}>
                <Text style={[styles.modalTitle, { color: textColor }]}>
                  {title}
                </Text>
              </View>)}
            <FlatList data={options} keyExtractor={(item, index) => `${item.value}-${index}`} renderItem={({ item }) => {
                const isSelected = item.value === value;
                return (<Pressable style={[
                        styles.option,
                        isSelected && { backgroundColor: isDark ? "#27272a" : "#f4f4f5" },
                        item.disabled && styles.optionDisabled,
                    ]} onPress={() => handleSelect(item)} disabled={item.disabled}>
                    ...
                    {isSelected && (<Text style={[styles.checkmark, { color: primaryColor }]}>
                        ✓
                      </Text>)}
                  </Pressable>);
            }} ItemSeparatorComponent={() => (<View style={[styles.separator, { backgroundColor: borderColor }]}/>)}/>
            <Pressable style={[styles.cancelButton, { borderTopColor: borderColor }]} onPress={() => setVisible(false)}>
    ...
    return (<>
      <Pressable style={[
            ...
            style,
        ]} onPress={openPicker} disabled={disabled}>
        <Text style={[
            styles.triggerText,
