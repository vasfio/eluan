...
export const Default = {
    render: function DefaultStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="Select Option" options={[
...
export const WithTitleAndMessage = {
    render: function WithTitleAndMessageStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="Delete Item" message="Are you sure you want to delete this item? This action cannot be undone." options={[
                {
                    key: "delete",
                    label: "Delete",
                    destructive: true,
...
export const WithIcons = {
    render: function WithIconsStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="File Actions" options={[
                {
                    key: "download",
                    label: "Download",
...
export const WithDescriptions = {
    render: function WithDescriptionsStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="Share" options={[
                {
                    key: "public",
                    label: "Public",
...
export const WithDisabledOptions = {
    render: function WithDisabledOptionsStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="Account" options={[
                {
                    key: "profile",
                    label: "Edit Profile",
...
export const DestructiveActions = {
    render: function DestructiveActionsStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} title="Delete Account" message="This will permanently delete your account and all associated data. This action cannot be undone." options={[
                {
                    key: "export",
...
export const PhotoOptions = {
    render: function PhotoOptionsStory() {
        ...
        <ActionSheet visible={visible} onClose={() => setVisible(false)} options={[
                {
                    key: "camera",
                    label: "Take Photo",
                    icon: <Text style={{ fontSize: 18 }}>📷</Text>,
...
export const MultipleSheets = {
    render: function MultipleSheetsStory() {
        ...
        <ActionSheet visible={activeSheet === "share"} onClose={() => setActiveSheet(null)} title="Share" options={[
                { key: "copy", label: "Copy Link" },
                { key: "email", label: "Email" },
                { key: "message", label: "Message" },
            ]}/>

        <ActionSheet visible={activeSheet === "edit"} onClose={() => setActiveSheet(null)} title="Edit" options={[
                { key: "rename", label: "Rename" },
                { key: "move", label: "Move" },
                { key: "duplicate", label: "Duplicate" },
            ]}/>

        <ActionSheet visible={activeSheet === "delete"} onClose={() => setActiveSheet(null)} title="Are you sure?" message="This cannot be undone." options={[
                { key: "delete", label: "Delete", destructive: true },
            ]}/>
      </View>);
    },
};
//# sourceMappingURL=ActionSheet.stories.js.map