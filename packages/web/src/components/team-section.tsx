...
>(({ className, ...props }, ref) => (
  <p
    ref={ref}
    className={cn(
      "mx-auto mt-[var(--spacing-md)] max-w-2xl text-[length:var(--font-size-lg)] leading-relaxed text-[color:var(--container-fg-alt)]",
      className
    )}
    {...props}
  />
  ...
  ({ className, alt, fallback, ...props }, ref) => (
    <div className="relative mx-auto mb-[var(--spacing-md)] aspect-square w-full max-w-[200px] overflow-hidden rounded-[var(--curves-xl)] bg-[var(--container-bg-alt)]">
      {props.src ? (
        ...
        <div className="flex h-full w-full items-center justify-center text-[length:var(--font-size-4xl)] font-medium text-[color:var(--container-fg-alt)]">
...
const TeamMemberLink = React.forwardRef<HTMLAnchorElement, TeamMemberLinkProps>(
  ({ className, label, children, ...props }, ref) => (
    <a
      ...
      aria-label={label}
      className={cn(
        "flex h-[var(--size-md)] w-[var(--size-md)] items-center justify-center rounded-[var(--curves-lg)] text-[color:var(--container-fg-alt)] transition-colors hover:bg-[var(--container-bg-alt)] hover:text-[color:var(--container-fg)]",
        className
      )}
      {...props}
    >
